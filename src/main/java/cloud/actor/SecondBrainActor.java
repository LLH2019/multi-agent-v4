...
public class SecondBrainActor extends AbstractBehavior<BasicCommon> {
    ...
    private List<ProcessInfo> haveAssignedTimes;

    private List<String> taskProcessNums = new ArrayList<>();
...
//        System.out.println("66666666666");
        ...
        bidingMsg.setTaskProcessNum(taskProcessNums.get(bind.getRound()));
        bidingMsg.setNo(bind.getRound());
        bidingMsg.setPreTime(bind.getPreTime());
        bidingMsg.setTaskName(refName);
//        System.out.println("handleBiding " + taskProcessNums.get(bind.getRound()) + " " + bind.getRound() + " " + bind.getPreTime());
...
//        List<ProcessTime> processTimes = new ArrayList<>();
        for (int i=0; i<precessNumbers.length; i++) {
            ...
            taskProcessNums.add(precessNumbers[i]);
//            BidingMsg bidingMsg = new BidingMsg();
