 ...
 * @description：计算加工成本
 */
public class CalculateHandleCost {
    ...
    public List<List<int[]>> getProcessCost(int taskNum, int rapid, int taskSize, int resourceSize) {
        ...
        try { // 防止文件建立或读取失败，用catch捕捉错误并打印，也可以throw
            ...
            /* 读入TXT文件 */
            String pathname ="D:\\Coding\\JavaProject\\multi-agent4\\data\\write-with-cost-by" + resourceSize + "-" + (taskSize*taskNum) +"-" + rapid +".txt"; // 绝对路径或相对路径都可以，这里是绝对路径，写入文件时演示相对路径
...
//            line = br.readLine();
            do {
                ...
                if (line != null) {
                    ...
                    totalCost += processCost.get(task).get(process)[resource] * (endTime-startTime);
//                    System.out.println(line);
    ...
    public static void main(String[] args) {
        ...
        CalculateHandleCost calculateHandleCost = new CalculateHandleCost();
